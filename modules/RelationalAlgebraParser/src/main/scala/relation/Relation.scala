...
class RelationHead(val atts: List[Attribute]) {
    if(atts.size > Properties("max_num_attributes").toInt) throw new IllegalArgumentException("Number of attributes in intermediate relation exceeds the maximum of " + Properties("max_num_attributes") + " attributes")

    def attribute(qualifiedAttName: String) = {
        val parts = qualifiedAttName.split("\\.").toList
        
        val (attName, o: Option[_]) = if(parts.length > 2) {
        ...
        val origin = o.asInstanceOf[Option[String]]
...
object Relation {
    ...
    def relationContentErrors(content: scala.util.parsing.input.StreamReader): Option[String] = {
        try {
            RelationParser.relation("name")(new RelationParser.lexical.Scanner(content)) match {
                case RelationParser.Success(rel, _) => None
                case RelationParser.Failure(msg, _) => Some(msg)
                case RelationParser.Error(msg, _) => Some(msg)
            }
        } catch {
    ...
    def relationContentErrors(content: String): Option[String] = {
        val inReader = scala.util.parsing.input.StreamReader(
            new StringReader(content)
        )
        relationContentErrors(inReader)
    }
}

object RelationBuffer {
    val maxNumTuples = Properties("max_num_tuples").toInt
}

class RelationBuffer(val relationHead: RelationHead) {
    val buffer = new mutable.HashSet[Map[Attribute, Any]]
    var numTuples = 0

    def += (tuple: Map[Attribute, Any]) = {
        if(numTuples == RelationBuffer.maxNumTuples) {
            throw new RuntimeException("Number of tuples exceeded the maximum of " + RelationBuffer.maxNumTuples + ".")
        } else {
            buffer += tuple
        }
        numTuples += 1
    ...
    def toRelFromAttributeTuples = new Relation(relationHead, relationHead.fromAttributeTuples(buffer.toSet))
    def toRel = new Relation(relationHead, buffer.toSet)
}
...
class Relation(val relationHead: RelationHead, val content: Set[Map[Attribute, Any]]) {
    ...
    def join(relation: Relation, cond: ((RelationHead, Map[Attribute, Any]) => Boolean)) = {
        ...
        val buffer = new RelationBuffer(relHead)
        for(
            tuple <- content; 
            tuple2 <- relation.content 
            if(cond(relHead, tuple ++ tuple2))) {
            buffer += (tuple ++ tuple2)
        }

        buffer.toRelFromAttributeTuples
    }
    ...
    def fullOuterJoin(relation: Relation, cond: ((RelationHead, Map[Attribute, Any]) => Boolean)) = {
        ...
        for(tuple <- relation.content) {
            ...
            if(joinPartnersForTuple.isEmpty) {
                buffer += (tuple ++ (relationHead.atts.map(att => att -> null)).toMap)
            }
        }
    ...
    def crossProduct(rel: Relation) = {
        ...
        val buffer = new RelationBuffer(newRelationHead)

        for(thisRelTuple <- content; otherRelTuple <- rel.content) {
            val tuple = thisRelTuple.map(value => thisRelHeaderMapping(value._1) -> value._2) ++ 
